/*
 * 07-binary-tree / p2.cpp
 * ------------------------------------------------------------
 * [문제] 이진 트리 순회 + 누적합 + LCA
 *   - 명령:
 *       Insert x v left|right
 *       Inorder_sum x : 중위(inorder) 순서로 값을 누적하다가 합이 x를 처음
 *                       초과하는 노드의 값을 출력(끝까지 못 넘기면 -1).
 *       Preorder_travel x : 전위 순서로 방문 노드를 출력하며 x를 만나면 멈춤
 *                       (경로상 방문값들이 출력됨, 못 찾으면 -1).
 *       LCA x y : 두 노드의 최소 공통 조상.
 * [핵심] 순회 함수가 bool을 반환해 "목표 도달 시 재귀 조기 종료".
 *        LCA는 각자 부모를 타고 루트까지 모은 뒤 처음 겹치는 조상을 찾음.
 */
#include <iostream>
#include <list>
#include <vector>
...
public:
	...
	bool inorder_sum(int value, Node* node, int& sum) {
		if (node->left != nullptr) {
			if (inorder_sum(value, node->left, sum)) return true;
		}
		sum += node->value;
		if (sum > value) {
			cout << node->value;
		...
		if (node->right != nullptr) {
			if (inorder_sum(value, node->right, sum)) return true;
		}
		return false;
	}
	bool preorder_travel(int value, Node* node) {
		cout << node->value << " ";
		if (node->value == value) {
			return true;
		}
		if (node->left != nullptr) {
			if (preorder_travel(value, node->left)) return true;
		}
		if (node->right != nullptr) {
			if (preorder_travel(value, node->right)) return true;
		}
		return false;
	}
...
int main() {
	...
	while (T--) {
		...
		for (int i = 0; i < Q; i++) {
			...
			else if (input == "Inorder_sum") {
				int x; cin >> x;
				int sum = 0;
				if (!t.inorder_sum(x,t.getRoot(),sum)) {
					cout << -1 << '\n';
				}
				else {
					cout << '\n';
			...
			else if (input == "Preorder_travel") {
				int x; cin >> x;
				if (!t.preorder_travel(x, t.getRoot())) {
					cout << -1 << '\n';
				}
				else {
					cout << '\n';
